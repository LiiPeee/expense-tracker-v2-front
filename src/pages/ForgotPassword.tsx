...
export default function ForgotPassword() {
  ...
  async function onSubmit(data: ForgotPasswordForm) {
    ...
    if (!result.ok) setErrorMessage(result.message ?? t("fallback.forgot"));
  }
    ...
    <div className="page-shell relative min-h-screen flex items-center justify-center px-4 py-10 overflow-hidden">
      ...
      <Card className="w-full max-w-md border-glass bg-card/80 shadow-strong reveal-up">
        <CardHeader className="text-center space-y-2">
          ...
          <CardTitle className="text-2xl">{t("recoverTitle")}</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormTextField control={form.control} name="email" label={t("email")} type="email" placeholder={t("emailPlaceholder")} />
              <LoadingButton type="submit" className="w-full" isLoading={form.formState.isSubmitting} loadingText={t("sending")}>
                {t("sendRecoveryCode")}
              </LoadingButton>
            </form>
          </Form>
